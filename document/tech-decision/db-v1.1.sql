-- ============================================================
-- FamSchedule v1.1 — Database Schema (PostgreSQL / Supabase)
-- ============================================================
-- 对应 ADR:
--   001  Supabase 后端
...
--   007  家庭共享 + 设备本地 混合
--
-- v1.0 → v1.1 变更摘要(参见 §5 RLS 编写规则):
--   1. RLS 无限递归修复 — 新增 public.my_family_ids() SECURITY DEFINER
--      函数,所有 11 个 family-scoped RLS policy 改用此函数,避免在
--      family_members 自身 policy 上自触发。
--   2. checkin_task id 歧义修复 — RETURNS TABLE 同名列(id / completed_at
--      / completed_by / is_makeup)与 public.tasks 同名冲突,UPDATE 改用
--      `public.tasks AS t` 表别名 + `t.` 前缀,消除 PL/pgSQL 解析歧义。
--   3. undo_checkin id 歧义修复 — 同上,防御性应用。
--
-- 部署:在 Supabase SQL Editor 中按顺序执行;支持 fresh deploy(idempotent)
...
-- 当前调用者所在的所有 family_id 集合(RLS 编写规则 #1,见 §5)
-- 关键:必须 SECURITY DEFINER + STABLE,绕开 family_members 自身 SELECT policy
-- 否则在 RLS 上下文里读 family_members 会触发该表的 SELECT policy,从而无限递归。
-- 另:本函数 auth.uid() 为 NULL 时返回空集,所有调用它的 RLS USING 表达式
-- 自动 deny,等价于 anon 角色 0 行可见(无需额外 anon-targeted policy)。
...
-- CREATE TRIGGER 不支持 IF NOT EXISTS,用 DO 包裹以保证 idempotent
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger
    WHERE tgname = 'trg_family_members_cap'
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger
    WHERE tgname = 'trg_task_templates_updated_at'
      AND tgrelid = 'public.task_templates'::regclass
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger
    WHERE tgname = 'trg_family_settings_updated_at'
      AND tgrelid = 'public.family_settings'::regclass
  ) THEN
...
-- 编写规则 #2:RETURNS TABLE 同名列必须用 `AS t` 表别名 + `t.` 前缀,
-- 否则 PL/pgSQL 解析 WHERE/SET 时的列名会与 RETURNS TABLE 同名列歧义,
-- 函数能 CREATE 但首次 CALL 必崩 "column reference X is ambiguous"。
...
-- 编写规则 #2 同上:虽然 undo_checkin 原始版本因为 RETURNS TABLE 列表不同
-- (没有 is_makeup)实际能跑通,但为了一致性和防御未来字段调整,统一用 t. 前缀。
...
-- ============================================================
-- 编写规则:
--   #1  RLS policy 严禁内嵌 self-table 子查询;统一通过 SECURITY DEFINER
--       函数间接读取。v1.0 中所有 family-scoped policy 用
--         family_id IN (SELECT family_id FROM public.family_members
--                       WHERE user_id = auth.uid())
--       会触发 family_members 自身 SELECT policy → 无限递归。
--       v1.1 起统一改用 public.my_family_ids() 函数(见 §2)。
--   #2  RETURNS TABLE 同名列名函数体必须用表别名 AS t 消除 PL/pgSQL 歧义。
--       见 checkin_task / undo_checkin(§4.5 / §4.6)。
...
-- ============================================================
-- 完毕。共 6 张业务表 + 7 个 RPC + 4 个 trigger 函数 + RLS 11 个 + 索引 10 条 + 唯一约束 1 条 + 视图 1 个
-- v1.1 与 v1.0 差异:新增 public.my_family_ids() 函数;
--   checkin_task / undo_checkin 用 AS t 消除 PL/pgSQL 同名列歧义;
--   11 个 RLS policy 改用 my_family_ids() 避免 family_members 自身递归;
