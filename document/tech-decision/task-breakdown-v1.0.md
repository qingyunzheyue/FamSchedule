...
| 最近更新 | 2026-09-22 — **T-US012-2 ✅ done**(生成邀请码 UI:`src/screens/InviteScreen.tsx` + 倒计时 hook `src/lib/inviteCountdown.ts` + `FamilyService.createInvite`;US-012 进度 1/3 → 2/3);T-US013-2 ✅(启动守卫 retry-once + SplashScreen 抽出 + 错误屏重试 UI;US-013 进度 1/2 → 2/2);T-US013-1 ✅(AuthService + SecureStore 持久化层抽离,AuthContext 从 236 行减到 196 行,15 个新单测全 pass);Wave 1 P0 修复 3/3 全清(T-FIX-01/02/03);§3.5 Review 修复任务新增 6 项(T-FIX-01..06)+ §9 Recommended Execution Order;源自 code-reviewer 对 SETUP 阶段的 NEEDS FIXES 评审(0 Critical / 3 Major / 11 Minor / 6 Cross-stack);3 个 P0 修复(T-FIX-01 同步数据丢失 / T-FIX-02 EAS projectId 一致性 / T-FIX-03 通知权限弹窗时机)**全部完成**,T-FIX-04..06 P1 留在 Sprint 2 Wave 2 主线收尾 |
...
| US-012 创建/加入家庭 | 3 | 2/3(**T-US012-1 ✅** FamilyService + FamilyContext 重构 + Gate 收敛 + **T-US012-2 ✅** 生成邀请码 UI + 倒计时 hook + createInvite RPC) | 🟡 |
| US-001 创建任务 | 2 | 0/2 | ⚪ |
| US-002 查看任务列表 | 3 | 0/3 | ⚪ |
