...
| 最近更新 | 2026-09-22 — **T-US012-3 ✅ done**(配偶输入邀请码 UI:`src/screens/JoinFamilyScreen.tsx` 6 格 TextInput + 自动跳转 + 粘贴 + 退格跳回 + `src/lib/codeInput.ts` 纯逻辑层 + `submitJoinCode` state machine + accept_invite RPC + 加入后 `SyncManager.subscribeFamily` 自动订阅;US-012 进度 2/3 → **3/3 ✅**;50 个新单测全 pass);T-US012-2 ✅(生成邀请码 UI + createInvite RPC;84 单测全 pass);T-US013-2 ✅(启动守卫 retry-once + SplashScreen 抽出 + 错误屏重试 UI);T-US013-1 ✅(AuthService + SecureStore 持久化层抽离);Wave 1 P0 修复 3/3 全清(T-FIX-01/02/03);T-FIX-04..06 P1 留在 Sprint 2 Wave 2 主线收尾 |
