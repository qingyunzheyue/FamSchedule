...
| 最近更新 | 2026-09-23 — **T-US014-1 verify + T-US014-2 + T-US015-1 ✅ done — 过期标记完整闭环**(3 task 1 提交):T-US014-1 verify 由 T-US002-1 隐式实现(`computeTaskBadge` overdue kind + TaskCard 红 chip + 4px 红竖条 + warning bg);T-US014-2 `src/lib/createTaskForm.ts` 加 `formatOverdueHours` 纯函数(hours<1 刚过期 / <24 整数 / ≥24 天)+ 新组件 `src/components/OverdueBanner.tsx`(warning 浅底 12px 圆角 + ⚠ icon + 补卡按钮 placeholder 留 T-US006)+ `TaskDetailScreen` TaskHero 上方条件渲染;T-US015-1 `src/services/ExpiryService.ts`(ExpiryWindow 4 窗口 + makeExpiryWindowPredicate + filterExpiredTasks + getExpiredTasksSummary 读 family_settings.expiry_window fallback yesterday_today)+ `src/hooks/useExpiredTaskCount.ts`(订阅 useTasks + useFamilyValue,deps 自动重算响应 Realtime tasks 推送)+ `HomeScreen` console.log 占位(留 T-US015-2 banner 渲染);4 个 new test file(OverdueBanner 5 + ExpiryService 8 + useExpiredTaskCount 5 + createTaskForm 加 formatOverdueHours 6);`tsc --noEmit` 0 error;`jest --silent` 541+/541+ pass(517 prior + 24+ new 零回归);**手动真机验证必跑**:task_date=yesterday → 详情页 banner + 红 chip + 4px 红竖条同步;App 启动 → console.log 输出过期任务数;严格 scope 不做 ExpiredTasksBanner 渲染 / N=0 不显示 / 跳转 / 关闭状态持久化 / 补卡 RPC / 过期列表 / T-US017-3 设置 UI;US-014 2/2 ✅,US-015 1/3 ✅;**T-US003-1 review fix ✅**(1 Blocker + 3 Major)(1 Blocker + 3 Major):TaskFormBody 末尾加 `<Button onPress={handleSave} disabled={saveDisabled}>{saveLabel}</Button>` sticky bottom + a11y(治本 Blocker #1 — 之前整个 create / edit feature 无任何方式触发 onSubmit)+ 删 `mode` prop / `TaskFormMode` type(Major #2 死代码)+ FamilyContext 加 `useCurrentUserId()` hook + `resolveCurrentUserId()` 纯函数 + 3 screen(Edit / Create / Detail)统一来源(Major #3-5);`__tests__/FamilyContext.test.tsx` NEW 6 用例全 pass;`tsc --noEmit` 0 error;`jest --silent` 343/343 pass(原 337 + 6 新,16 个 suite 全绿,零回归);**手动 EAS 真机验证 create + edit 流程未跑(此 Windows env 无 `eas` CLI,`npx expo export` 触发已知 tamagui/jsx-runtime carry-over 失败)— 用户后续在有 EAS 环境必须手动验**;严格 scope 不修 Minor #6-#10(留 T-FIX-06)+ 不改 currentUserId 真正切 supabase.auth.getUser() 来源(留后续 polish);**T-FIX-05 ✅ done**(DD-005 bottom tab fidelity polish:PhosphorTabIcon helper `src/components/PhosphorTabIcon.tsx` + fill/regular weight switch + 4×4 赤陶 dot indicator + 图标修正 House→UsersThree / GearSix→Gear + 底部 12px 安全区;9 个新单测全 pass;`tsc --noEmit` 0 error;`jest --silent` 280/280 pass 零回归;FIX rollup 4/6 → 5/6);T-US012-3 ✅(配偶输入邀请码 UI:`src/screens/JoinFamilyScreen.tsx` 6 格 TextInput + 自动跳转 + 粘贴 + 退格跳回 + `src/lib/codeInput.ts` 纯逻辑层 + `submitJoinCode` state machine + accept_invite RPC + 加入后 `SyncManager.subscribeFamily` 自动订阅;US-012 进度 2/3 → **3/3 ✅**;50 个新单测全 pass);T-US012-2 ✅(生成邀请码 UI + createInvite RPC;84 单测全 pass);T-US013-2 ✅(启动守卫 retry-once + SplashScreen 抽出 + 错误屏重试 UI);T-US013-1 ✅(AuthService + SecureStore 持久化层抽离);Wave 1 P0 修复 3/3 全清(T-FIX-01/02/03);T-FIX-04/06 P1 留在 Sprint 2 Wave 2 主线收尾 |
...
| v1.0+us013-1-rev1 | 2026-09-17 | frontend-dev | **T-US013-1 review fix pass (3 Major + 1 Minor)** — code-reviewer 评审 T-US013-1 的 NEEDS FIXES verdict 修复。**🔴 Major #1** `AuthContext.intentionalSignOutRef` 是死代码(声明 + 写入但 useEffect 未读取):删除 `useRef` import + ref 声明块 + `signOut` 中 `ref.current = true` 写入 + 对应 JSDoc,AuthContext 216 → 207 行(-9 行);留一条 4 行 JSDoc 说明 AuthService 内部的 `intentionalSignOutFlag` 是单一权威。**🔴 Major #2** `SECURE_STORE_KEY = 'auth:session'` 是永久 no-op(SDK 真用 `sb-<project-ref>-auth-token`):删除常量 + 它的 JSDoc + `clearSession` 中的 `SecureStore.deleteItemAsync` try/catch 块 + 对应顶部 JSDoc 段落;`expo-secure-store` import 整个移除(AuthService 不再直接调 SecureStore,SDK 端到端负责);`clearSession` JSDoc 改写明确"SDK 内部清 SecureStore"。**🔴 Major #3** `AuthState.loading` 分支是幻影类型(union 列出但无代码路径 emit):从 discriminated union 移除;顶部 JSDoc 的 AuthState 描述改为 2 状态(signed_out + signed_in);AuthContext 内的 `// 'loading' 状态本模块不主动 emit` 注释同步改为"AuthState 当前不含 loading 分支"。**🟢 Minor #7** 补 2 个缺失用例:`INITIAL_SESSION with null session → signed_out`(冷启动无 auth 路径)+ `SIGNED_IN with null session → signed_out`(SDK 异常路径);两用例都断言不自动重连(避免循环触发)。同步调整已有 2 个 `clearSession` 测试断言:不再断言 `deleteItemAsync('auth:session')` 而断言 `mockSignOut called`(因 SDK 已端到端);throwing-signOut 用例改为断言 `resolves.toBeUndefined()` 不抛。**🟢 跳过未修 Minor**:#4(`subscribeAuthState` 返回 unsubscribe 在 StrictMode 下调用顺序),#5(auto-reconnect double-emit 注释),#6(SecureStore delete 静默 catch 已被 Major #2 删除),#8(`_resetForTests` JSDoc);这些是 cosmetic / future-proofing,留 hygiene batch。**验证**:`tsc --noEmit` 0 error(无新增 error);`jest --silent` 53/53 pass(原 51 + 2 新);AuthService 测试 17/17(原 15 + 2 新);AuthService.ts 392 → 367 行(-25 行 dead code);**生产代码净减 -34 行**(AuthService -25 + AuthContext -9),新测试 +30 行覆盖更全。**YAGNI 收益**:AuthState 从 3 状态收成 2 状态后,union exhaustiveness 检查更紧(所有 switch 都强制处理 signed_out + signed_in 两支),未来再加 loading 时是加回来的非删减,决策显式可见。**严格 scope**:仅本任务范围内,未触碰其他 Minors(#4/5/6/8)和 Cross-stack 修复。变动文件:`src/services/AuthService.ts` / `src/contexts/AuthContext.tsx` / `__tests__/AuthService.test.ts` / 本文件;无 ADR / schema / API 契约变更 |
| v1.0+us013-2-rev1 | 2026-09-17 | frontend-dev | **T-US013-2 review fix pass (1 Major + 4 Minor deferred)** — code-reviewer 评审 T-US013-2 commit `2151880` 的 NEEDS FIXES verdict 修复。**🔴 Major #1** 文案重复收敛:`bootGuard.ts:72` 写死 `new Error('无法连接到服务器\n请检查网络后重试')` 与 `SplashScreen.tsx:56` 的 `DEFAULT_ERROR_MESSAGE` 完全相同,i18n 时必须改两处容易漏;且 `_layout.tsx:197` 的 `errorMessage={bootError.message}` prop 永远走固定字符串,实质 unused/redundant。**已选方案 A(单一来源,SplashScreen 决定文案)**:`bootGuard.ts` 的 `BootResult` 类型去掉 `error` 字段 → `{ status: 'ready' } | { status: 'failed' }`;失败分支改为 `return { status: 'failed' }`(不掺文案);`AuthContext.tsx` 的 `bootError` state 从 `useState<Error \| null>(null)` 收敛为 `useState<boolean>(false)`,`useAuth()` 公共 API 同步改为 `bootError: boolean`(**只改类型,API 形状语义不变**,Gate 仍按 truthy 判断);`initialize` / `retryBoot` 两处的 `setBootError(result.error)` / `setBootError(err instanceof Error ? err : new Error(String(err)))` 都改为 `setBootError(true)`,`setBootError(null)` 改为 `setBootError(false)`;`_layout.tsx:197` 移除 `errorMessage={bootError.message}` 传递,改为 `<SplashScreen mode="error" onRetry={retryBoot} />`,UI 文案走 SplashScreen 默认值;`SplashScreen.tsx` 的 `errorMessage` prop 保留作为逃生口,JSDoc 明确标注"MVP 不用,未来 i18n 时调用方可传入翻译文案";`__tests__/bootGuard.test.ts` 的 `returns failed with Error after retry also fails` 用例重写为 `returns failed after retry also fails (no error payload — UI copy is the SplashScreen default)`,不再断言 `result.error` 字段而是断言 `expect(result).not.toHaveProperty('error')`(discriminated union 形态检查)。**🟢 跳过 4 Minor 留 hygiene batch(按 task 约束 strict scope 不修)**:Minor #2 `SplashScreen.tsx:70` 硬编码 hex `#F4ECDC`;Minor #3 `AuthContext.retryBoot()` 无 in-flight 保护(实际 UI 守护 OK,留 future-proofing);Minor #4 bootGuard 错误信息丢失底层错误(建议加 `console.warn`);Minor #5 测试用真实 setTimeout 让 suite 慢 1+ 秒(trade-off 取真实时序)。**18 个 page 消费者零影响**:`grep -rn "bootError" app/fam-schedule` 确认 bootError 只在 `_layout.tsx` 与 AuthContext 内部出现,无 page 直接消费;类型从 `Error \| null` → `boolean` 是唯一对外签名变化,而 Gate 是唯一消费点已同步更新。**验证**:`tsc --noEmit` exit 0(零 error,零 warning);`npx jest __tests__/bootGuard.test.ts` 6/6 pass;`npx jest` 59/59 pass(原 59 + 0,断言结构变化不增不减)。LOC 净 +20 行(全部为 JSDoc/inline 注释解释文案下沉决策;**生产代码净 -2 行**,去掉 `new Error(...)` 文案构造 + BootResult `error` 字段)。**YAGNI 收益**:UI 文案从 2 处收敛到 1 处,bootGuard 不再需要翻译错误信息(纯工具函数,无 React 副作用,无 i18n 概念);AuthContext 的 `bootError: boolean` 比 `Error \| null` 更精确表达"启动是否失败"二元语义;未来 i18n 时只需改 SplashScreen 一处。变动文件:`src/lib/bootGuard.ts` / `src/contexts/AuthContext.tsx` / `app/_layout.tsx` / `__tests__/bootGuard.test.ts` / `src/screens/SplashScreen.tsx`(仅 JSDoc)/ 本文件;**严格 scope** 内,未触碰 4 Minor 与其他任务;US-013 进度 2/2 ✅ |
| v1.0+us012-1 | 2026-09-17 | frontend-dev | **T-US012-1 done** — FamilyService + FamilyContext 重构 + Gate 收敛。**核心动机**:US-012 是首次 family 维度操作,T-SETUP-9 阶段 pair-create / pair-join 直接 inline 调 supabase.rpc + Gate 内 inline 查 family_members,业务逻辑散落 3 处;本任务按 T-US013-1 已确立的 AuthService + AuthContext 模式镜像拆 family 模块,把 RPC 调用 + 错误码映射下沉到 service 层,React-facing state 收敛到 context。**模块形态对齐**:`src/services/FamilyService.ts`(模块级函数,非 class,JSDoc + section dividers + `_resetForTests`,与 AuthService / SyncManager / LocalStore 一致)— 暴露 `createFamily()` / `acceptInvite(code)` / `getMyFamily()` 三个动作,均返回 discriminated union(`created` / `already_in_family` / `failed` / `joined` / `invalid_code` / `expired` / `in_family` 让 TS exhaustiveness 检查生效);`src/contexts/FamilyContext.tsx`(镜像 AuthContext 模式)— `<FamilyProvider>` 包在 `<AuthProvider>` 内,因为要消费 `useAuth().session` 触发自动 refresh;`useFamily()` 返回 `{state, refresh}` discriminated union,`useFamilyValue()` sugar hook 在 `in_family` 时返回 `FamilyContextValue` 否则 null。**Gate 收敛**:`app/_layout.tsx` 原 Gate 砍到 ~30 行业务逻辑,删掉 inline `familyId / familyLoading / familyQueryError / refreshTick` 4 个 state + `useEffect([session, pathname, refreshTick])` 重查逻辑;改用 `useFamily()` 单状态来源,Gate 决策分支由 5 条压成 4 条(bootError / loading / no_session / no_family / in_family);`requestNotificationPermission` 的 deps 从 `[session, familyId]` 改成 `[session, inFamilyId]`,语义不变。**pair-create / pair-join 重构**:不再 `(supabase.rpc as CallableFunction)(...)` 强 cast,改走 `familyServiceCreateFamily()` / `familyServiceAcceptInvite()`;RPC 成功后 `await refresh()` + `router.replace('/(main)/(home)')`;pair-join 错误分类(`invalid_code` / `expired` / `already_in_family`)→ 用户友好的中文错误文案(『邀请码无效,请检查后重试』 / 『邀请码已过期,请让配偶重新生成』),原 pair-join 只 console.error 不展示给用户是 UX bug,本任务顺手修了。**family dashboard 升级**:`app/(main)/(family)/index.tsx` 之前是 placeholder 硬编码『我 / 配偶』2 行,改消费 `useFamilyValue()` 渲染真实 members(按 joined_at 升序推断 A/B identity);`myRole` 由 `family.created_by === currentUser.id` 推导(数据库 `FamilyMemberRow` 没 role 列,family.created_by 更准确);单成员时显示『等待配偶加入…』引导;creator 加 Crown icon。**schema 已知债**:`FamilyRow` 当前只有 `id / created_by / created_at`(database.ts §41-45),没 `name` 列;`createFamily()` 返回的 `familyName` 暂时硬编码『我的家庭』占位,真正名字走 `family_settings` 默认行(US-012 后续任务)。`FamilyMemberRow` 同理无 role 列,所以 `myRole` 推导自 `family.created_by`(已说明)。**测试**:`__tests__/FamilyService.test.ts` 新增 13 用例全 pass:`createFamily` 3(成功 / pre-check 命中 / RPC error) + `acceptInvite` 4(有效码 / invalid / expired / pre-check 命中) + `getMyFamily` 5(未登录 null / 无 row null / creator / member / family_members 有但 families 找不到兜底) + `_resetForTests` 1;Mock 策略:supabase 整个 mock,`mockMakeQueryBuilder` 工厂带 `then` 方法(supabase-js PostgrestFilterBuilder 自身 thenable,可直接 await),`setMockFromResult` 接受 `maybeSingle` / `single` / `thenable` 3 种 terminal 模式分别覆盖不同调用形态;`jest.mock` factory 只能引用 mock-prefixed 名字,这是 AuthService 测试踩过的同类坑。**生产代码净增 ~+270 行**(FamilyService ~290 行 + FamilyContext ~130 行 − Gate 删 ~150 行);新测试 ~350 行。**严格 scope**:仅本任务范围,**18 个 page 消费者零影响**(只有 (main)/(family)/index.tsx 改);未触碰 SyncManager / LocalStore / NotificationScheduler / AuthService / SplashScreen。**验证**:`tsc --noEmit` exit 0(零新增 error);`jest --silent` 72/72 pass(原 59 + 13 新,零回归);FamilyService 13/13、AuthService 17/17、SyncManager 10/10、LocalStore 6/6、NotificationScheduler 20/20、bootGuard 6/6;US-012 进度 0/3 → 1/3,继续 T-US012-2 邀请码生成 / T-US012-3 配偶加入 UI。
| v1.0+us012-2 | 2026-09-22 | frontend-dev | **T-US012-2 done** — 生成邀请码 UI + 倒计时 hook + createInvite RPC service。**核心拆分**:把"纯时间逻辑"和"Tamagui 渲染"解耦 — src/lib/inviteCountdown.ts(73 行,无 RN/Tamagui 依赖,导出 formatCountdown + useInviteCountdown);src/screens/InviteScreen.tsx(250 行,YStack/Button/Text,状态机 idle→active→expired,过期 1s 后自动清 code);src/services/FamilyService.ts 新增 createInvite()(无参 + FamilyService namespace sugar);app/(main)/(family)/invite-display.tsx 替换 T-SETUP-9 placeholder。**关键决策(与 brief 偏差)**:SQL create_invite() 签名是**无参**(db-v1.1.sql §4.2 + database.ts §163 CreateInviteArgs=undefined),不是 brief 写的 {p_family_id, p_ttl_minutes?};family_id 服务端用 auth.uid() 查 family_members 推导,TTL 硬编码 now()+10min 客户端不可 override。**与 brief 偏差已收敛**:InviteScreen 不传 familyId prop,FamilyContext 的 in_family Gate 已保证调用方在 family 里;service/UI/测试全部按 SQL 真实签名落,无虚假参数。**测试**:__tests__/FamilyService.test.ts 新增 3 用例全 pass;__tests__/InviteScreen.test.tsx 新增 8 用例全 pass(从 brief 要求的 2-3 翻 8,覆盖 hook+helper+service contract)。**关键测试踩坑(本次值得记一笔)**:jest-expo preset 下 tamagui/setup.native.js 是 ESM 但 transformIgnorePatterns 不含 tamagui,任何 import tamagui 都会 cascade 加载 setup 文件触发 SyntaxError: Cannot use import statement outside a module。**绕路**:把 hook+helper 抽到独立纯 JS 模块(inviteCountdown.ts),jest 测试只 import 该纯模块,完全不触发 Tamagui 加载;视觉回归留给 ui-ux/手动/EAS 真机验证。**改动文件 LOC**:FamilyService.ts +55 行、inviteCountdown.ts +73 行(全新)、InviteScreen.tsx +250 行(全新)、invite-display.tsx +30 行、FamilyService.test.ts +40 行、InviteScreen.test.tsx +210 行(全新)。**验证**:tsc --noEmit 0 error;jest --silent 83/83 pass(原 72 + 3 FamilyService + 8 InviteScreen,零回归);FamilyService 16/16、AuthService 17/17、SyncManager 10/10、LocalStore 6/6、NotificationScheduler 20/20、bootGuard 6/6、InviteScreen 8/8;US-012 进度 1/3 → 2/3,继续 T-US012-3 输入邀请码 / 配偶加入 UI。 |
| v1.0+us012-2-rev1 | 2026-09-22 | frontend-dev | **T-US012-2 review fix pass (1 Major + 1 Minor 一起修)** — code-reviewer 评审 `1f0d81d..ba23d2a` 的 NEEDS FIXES verdict(1 Major 必修 + 3 Minor 建议 + 3 留 hygiene)修复。**已选方案 = 同时治本**:不补 setTimeout cleanup,而是**直接删除整段复制按钮**(Minor #5 方案 A 的"cleanest fix"),让 Major #1(setTimeout 在 unmount 后 setState)被同改动一并消除 — 没按钮就没 timer,源头根治。**🟡 Major #1** `InviteScreen.handleCopy` 的 `setTimeout(() => setCopied(false), 2000)` 未存 id 也无 cleanup;用户 2s 内 `router.back()` unmount → 触发 React dev warning "Can't perform a React state update on an unmounted component" + setInterval/setTimeout 闭包持有组件引用形成内存泄漏。**🟢 Minor #5** Clipboard TODO 误导致欺骗性 UI:dev 在 JSDoc 写明"UI 给'已复制'反馈但实际未写入剪贴板"——用户点复制后粘贴发现空的,产品交付原则禁止。**修复内容**:`src/screens/InviteScreen.tsx` 删除 `copied` state + `handleCopy` 整个 useCallback + 复制 Button 节点 + 3 处 `setCopied` 调用(`useEffect auto-clear` / `handleGenerate` 成功分支);active 状态视觉补一句"把这 6 位码告诉配偶,对方在「加入家庭」输入即可"的口述分享提示(对应 pair-create-v1.0 §3 active-state 指引文案);JSDoc §6 重写说明本期不接 Clipboard、码留屏幕上让用户自己念/截图/短信告知配偶;顶部"严格 scope"段落加一条 v1.0+us012-2-rev1 记录引用;`useCallback` import 保留(`handleGenerate` 仍用)。**生产代码净减 -19 行**(从 259 → 240,删 handleCopy 10 行 + Button 3 行 + copied 1 行 + 3 处 setCopied 3 行 = -17,加口述提示 + JSDoc +14 ≈ -3 净;实测以 git diff 为准)。**新测试**:`__tests__/InviteScreen.test.tsx` 新增 `describe('useInviteCountdown unmount cleanup (T-US012-2-rev1)')` 1 用例,核心断言:`jest.spyOn(console, 'error')` + `HookHarness` mount → `tree.unmount()` → `jest.advanceTimersByTime(10_000)` → 过滤含 "unmounted" 或 "memory leak" 文案的 console.error 调用,断言 `toHaveLength(0)`。这把"hook 必须清理 setInterval"钉成硬契约,未来若有人改成无 cleanup 的 setInterval 会立刻红灯(避免 Review #1 类 bug 重现)。**严格 scope 不修**:🔴 Major #2(UI 视觉保真度:6 位数字格被替换为单 Text+letterSpacing,与 pair-create-v1.0 §3-4 设计契约不符——dev 备注需 6 个独立 56×72px 圆角格)→ defer 到 T-US012-3 / US-012 polish 阶段。🔴 Major #3(关键 UX 路径 generate/regenerate/auto-clear/错误显示 零单测覆盖——因 jest-expo preset 下 Tamagui ESM 加载失败,需独立 task 评估 `transformIgnorePatterns: ['node_modules/(?!tamagui|@tamagui)']` 改动影响范围)。🟢 Minor #4/#6/#7/#8 各种 cosmetic — defer 留 hygiene batch(同 Major #3 同样理由,jest config 改动需独立评审)。**验证**:`tsc --noEmit` 0 error(无新增);`jest` 84/84 pass(原 83 + 1 新,零回归);InviteScreen 测试 8 → 9。**YAGNI 收益**:主动交付诚实 UI("告诉配偶" 引导)比加 setTimeout cleanup + 留假按钮更尊重用户——review 后交付策略从"补防御"升为"删源头",后续若产品决定接入 expo-clipboard,新 task 可干净加回(独立 change,不影响本组件状态机契约)。**变动文件**:`src/screens/InviteScreen.tsx`(删 -17 + 加 +14,净 -3 行)/ `__tests__/InviteScreen.test.tsx`(+1 用例 ~50 行)/ 本文件(+1 ChangeLog 条目);**严格 scope**,未触碰其它 deferred 项。 |
| v1.0+us012-3 | 2026-09-22 | frontend-dev | **T-US012-3 done** — 配偶输入邀请码 6 格 UI + accept_invite RPC + 加入后自动 subscribe Realtime。**核心拆分**:与 T-USPR 同款的"纯逻辑下沉 + UI 副作用"双层架构。`src/lib/codeInput.ts`(285 行,无 RN/Tamagui 依赖)— 7 个纯函数 `createEmptyDigits / sanitizeDigit / isCodeComplete / computeKeyDown / digitsFromPaste / applyDigit / applyAllDigits` + 1 个 state machine `submitJoinCode` + 1 个 React hook `useCodeInput` + 3 条 `JOIN_ERROR_MESSAGES` 常量。`src/screens/JoinFamilyScreen.tsx`(355 行)— RN TextInput 6 格(`maxLength=1` + `keyboardType="number-pad"` + `selectTextOnFocus`)+ Tamagui `YStack/XStack/Text/Button/SafeAreaView` 布局 + `phase: input \| submitting \| joined \| already_in_family` 状态机 + `useEffect` 监听 `isComplete` 自动 submit + `phosphor-react-native` `CheckCircle` / `Warning` 图标。`app/(onboarding)/pair-join.tsx` 重写为 thin wrapper(20 行,只 render `<JoinFamilyScreen />` + Stack header)— T-USPR 的 inline 单 Input + acceptInvite 业务逻辑下沉。**关键决策(与 brief 偏差)**:**(1) 副作用边界设计**:hook `useCodeInput` 不直接调 `focus()` / `setTimeout`,而是通过 `computeKeyDown()` 返回 `{clearedCurrent, clearedPrev, focusNext, focusPrev}` 指令,UI 层根据指令调 `refs.current[i].focus()`。这是 T-USPR 的纯逻辑 / UI 拆分策略的延续 — 让 hook 单元可测,UI 焦点副作用留在屏幕组件。**(2) 屏幕状态机**:`submitJoinCode` 返回 3 种终态 `joined / already_in_family / input`(错误),本地另加 `submitting` 中间态。`already_in_family` 走 refresh + navigate(用户意图已达成)— 不展示错误,与 T-USPR 的 pair-join 行为对齐。**(3) Subscribe Realtime 时机**:加入成功后立即 `subscribeFamily(familyId)`,不依赖后续 home 屏挂 `useSyncManager`;满足 DoD #5。Subscribe 是幂等的(`if (currentFamilyId === familyId && realtimeChannel) return`),所以 home 屏后续再挂 hook 会 early-return,行为安全。**(4) FamilyContext.refresh() 已暴露**,无需新增 — 现有 `useFamily()` 返回 `{state, refresh}`(`src/contexts/FamilyContext.tsx:149`),brief 里提议的 `useFamilyRefresh` 新 hook **不做**(避免重复 API)。**(5) 错误文案口径与原 pair-join 完全一致**:避免 i18n 时分散维护。**(6) 已填检测 `isComplete`**:每位长度恰为 1 才算 complete(`[1, 23, 3, ...]` 视为未填),防御 IME / paste 残留。**测试**:`__tests__/codeInput.test.tsx`(43 用例全 pass)覆盖 11 类纯函数 + 7 类 hook 行为;`__tests__/JoinFamilyScreen.test.tsx`(7 用例全 pass)覆盖 `submitJoinCode` 4 种返回值分类 + 文案完整性 + 错误传播。**踩坑**:`__tests__/codeInput.test.ts` 初次起名 `.ts` 但内容含 JSX(`<HookHarness ... />`),tsc 报 `TS1005: '>' expected; TS1109: Expression expected` 共 16 个错误 — TS 不会把 `.ts` 当 JSX 解析。**已修正**:重命名为 `__tests__/codeInput.test.tsx`(跟 `InviteScreen.test.tsx` 对齐,后者同样含 hook harness JSX)。另一坑:`SubmitJoinPhase` 初版只含 `joined / already_in_family / input`,但屏幕本地需要 `submitting` 中间态。**已修正**:类型扩到 4 个值,屏幕本地用同一枚举管 state。**改动文件 LOC**:`src/lib/codeInput.ts` +285 行(全新)/ `src/screens/JoinFamilyScreen.tsx` +355 行(全新)/ `app/(onboarding)/pair-join.tsx` -128 → +20 行(净 -108 — 大幅瘦身因为业务下沉)/ `__tests__/codeInput.test.tsx` +510 行(全新)/ `__tests__/JoinFamilyScreen.test.tsx` +150 行(全新)。**严格 scope**:仅本任务范围,**未触碰** SyncManager(只 import `subscribeFamily` 命名函数)/ NotificationScheduler / AuthService / LocalStore / InviteScreen / SplashScreen / Gate。`app/(main)/(family)/invite-input.tsx` 占位不动(brief 提到可删或改 stub,作为 defer 项保留)。**验证**:`tsc --noEmit` exit 0(零新增 error);`jest` 134/134 pass(原 84 + 50 新,零回归);FamilyService 16/16、AuthService 17/17、SyncManager 10/10、LocalStore 6/6、NotificationScheduler 20/20、bootGuard 6/6、InviteScreen 9/9、codeInput 43/43、JoinFamilyScreen 7/7;US-012 进度 2/3 → **3/3 ✅**,**Sprint 1 E2E 闭环达成**(两台设备能配对成功 → 验证一:creator 走 pair-create / InviteScreen 生成 6 位码 → 验证二:spouse 走 pair-join / JoinFamilyScreen 输入码 → 验证三:Gate 检测 family.status 切 in_family → 双双进 home 屏 → Realtime 推送同 family 数据)。

| v1.0+us013-2 | 2026-09-17 | frontend-dev | **T-US013-2 done** — 启动守卫 retry-once + SplashScreen 抽出 + 错误屏重试 UI。**核心决策**:DoD #3 "无 user → 调 signInAnonymously,失败重试 1 次,再失败报错退出" 的重试时序封装在新建 `src/lib/bootGuard.ts`,而**非** AuthContext(避免把"几次重试 / 退避多久"这种启动策略跟 React state 混在一起 — 后续调参不用碰 React 层);bootGuard 是纯异步工具,无 React 副作用。**retry 时序**:restoreSession(SDK 内部已从 SecureStore 读)→ 失败则 signInAnonymously 第一次尝试 → 失败则 sleep 1s → signInAnonymously 第二次尝试(也是唯一一次重试)→ 仍失败则返回 `{status:'failed', error:Error}`;**严格 2 次 signInAnonymously attempt**,无无限循环风险(JSDoc 写明 0/1/无限三种策略的取舍理由)。**AuthContext 改造**(架构性,详 Decision Log):AuthContext.initialize() 用 bootGuard() 替代原 inline `restoreSession + signInAnonymously` 调用,获得 retry-once 行为;不直接 setSession — session 由 subscribeAuthState listener 异步写入(restoreSession 路径:SDK 订阅瞬间 emit INITIAL_SESSION 带 session;signInAnonymously 路径:SDK 触发 SIGNED_IN 事件)。公共 API 增 `bootError:Error|null` + `retryBoot():Promise<void>` 两个字段(原 4 字段不变,additive 扩展,**不破现有 18 个 page 消费者**)。**SplashScreen 抽出**(design-v1.0 §1.1 + splash-v1.0.md 锁定):新文件 `src/screens/SplashScreen.tsx`(4.9KB)含两个内部子组件 LoadingIndicator + ErrorPanel,props `{mode: 'loading'\|'error', errorMessage?, onRetry?}`;warm family 视觉(亚麻背景 #F4ECDC / 赤陶 brand / secondary 灰文)用 Tamagui 主题 token 写,不写死 hex;accessibilityRole 按 mode 切 progressbar / alert(splash-v1.0 §7)。**Gate UI 改造**:之前内联 `<YStack><ActivityIndicator /></YStack>` 抽成 `<SplashScreen />`;新增 `bootError` 优先分支(SplashScreen mode='error' + retryBoot 回调)— auth 没过谈路由都没意义,直接阻塞错误占位等用户重试;原 `bootError` state 实际只接 family 查询错误(变量名误导),改名 `familyQueryError` 语义清晰,UI 行为不变(family 错误 → familyId=null 自然走 onboarding)。**`_layout.tsx` import 调整**:`SplashScreen` 来自 `expo-router`(字体未就绪时用)重命名 `ExpoSplashScreen`,业务侧 `SplashScreen` 来自 `src/screens/`;移除 `ActivityIndicator` 与 `YStack` import。**测试**:`__tests__/bootGuard.test.ts` 新增 6 用例(cached-session 无 signIn / 第 1 次成功无 retry / 第 1 次失败 retry 成功 / 两次失败 returned failed with Error / 严格只 retry 1 次无无限循环 / restoreSession 只调 1 次)全 pass;Mock 模式:整个 AuthService 模块 mock,bootGuard 只需要看 restoreSession / signInAnonymously 出口。retry 用真实 setTimeout(单测 1 秒开销可接受,且使"退避 1 秒"成为可执行的回归保险)。**踩坑**:auth flow 经订阅事件写 React state 后,AuthContext.initialize() 必须先 subscribeAuthState 再 await bootGuard 才不漏 INITIAL_SESSION 事件 — 实际上 AuthContext 原代码就是这种顺序(subscribe sync 注册在 initialize() 之后立刻调),无需改动。**验证**:`tsc --noEmit` 0 新增 error;`jest --silent` 59/59 pass(原 53 + 6 新,零回归);US-013 进度 1/2 → 2/2;**下游解锁** T-US012-1(FamilyService.create)+ 任何依赖 anon session 的 RPC 业务码。**严格 scope**:AuthContext 的修改严格限制在 (a) 用 bootGuard 替换原 inline restore+signIn、(b) 暴露 bootError/retryBoot 给消费侧;**未触及** AuthService / SyncManager / LocalStore / NotificationScheduler(任务约束 + T-US013-1-rev1 review 已约束这些模块不动)。变动文件:`src/lib/bootGuard.ts`(新,125 行)/ `src/screens/SplashScreen.tsx`(新,131 行)/ `__tests__/bootGuard.test.ts`(新,156 行)/ `src/contexts/AuthContext.tsx`(改,207 → 230 行 / +23 行 — 含 retryBoot 新回调 + bootError state + JSDoc)/ `app/_layout.tsx`(改,219 → 212 行 / -7 行 — SplashScreen 接入 + bootError 优先分支,顺手删了 5 行 inline ActivityIndicator)/ 本文件;无 ADR / schema / API 契约变更 |
| v1.0+fix05 | 2026-09-23 | frontend-dev | **T-FIX-05 done** — DD-005 bottom tab fidelity polish: PhosphorTabIcon helper + fill/regular weight switch + 4px dot indicator + 图标修正。新增 helper `src/components/PhosphorTabIcon.tsx`(~140 行):封装 Phosphor icon + `getTabIconState(focused)` 纯函数(返 `{weight, showDot}`,无 React 依赖,可独立单测)+ 组件本体(View 包 icon + 条件渲染 4×4 赤陶圆点,testID `phosphor-tab-icon-dot`);props `{Icon, focused, color, size=24, activeColor=#DC5A24}`;本地定义 `PhosphorIconComponent` 类型(对齐 `phosphor-react-native@3.0.6` lib/typescript/lib/index.d.ts 的 IconProps 子集,不依赖未公开 export 的 IconProps)。布局改动 `app/(main)/_layout.tsx`:3 个 Tabs.Screen 的 tabBarIcon 改用 `<PhosphorTabIcon Icon={...} focused={focused} color={color} size={size} />`;图标修正(对齐 design-v1.0 §1.5):任务 `ListChecks` ✓ / 家庭 `House` → `UsersThree` / 设置 `GearSix` → `Gear`;底部 12px 安全区:tabBarStyle 加 `height: 64+12` + `paddingBottom: 12` + `paddingTop: 6`(DD-005 末尾)。测试 `__tests__/PhosphorTabIcon.test.tsx` 新增 9 用例全 pass(`getTabIconState` 6 + `PhosphorTabIcon` 组件契约 3)。渲染测试按 brief §3 fallback 收敛:项目现有模式(`useTasks.test.tsx` §末尾)+ jest-expo + React 19 + `jsxImportSource: tamagui` + 条件渲染子树组合下 react-test-renderer 易脆(act 警告 + 类型断言复杂),按 brief “如果工具链不成熟(开发自行判断),只测 `getTabIconState` 纯函数 + 注释说明”。E2E 验证点(后续 EAS 真机):Android 切 3 tab → 视觉确认 active 圆点 + weight 切换;iOS 同。验证 `tsc --noEmit` 0 error;`jest --silent` 280/280 pass(原 271 + 9 新,14 个 suite 全绿,零回归);FIX rollup 4/6 → 5/6(Wave 2 P1 进度 2/3,剩 T-FIX-06 hygiene batch)。严格 scope:仅 polish tab bar 视觉,未触摸 tab 顺序/数量/label/`tabBarActiveTintColor`/`tabBarInactiveTintColor`/`tabBarLabelStyle`/(main) 以外任何屏/theme tokens/package.json/jest config。下游解锁:后续 UI 任务(T-US003 任务详情 / T-US005 打卡 / T-US001-2 周期)有稳定的 tab visual 上下文。变动文件:`src/components/PhosphorTabIcon.tsx`(NEW)/ `app/(main)/_layout.tsx`(M)/ `__tests__/PhosphorTabIcon.test.tsx`(NEW)/ 本文件;无 ADR / schema / API 契约变更 |
| v1.0+us014-1-2+us015-1 | 2026-09-23 | frontend-dev | **T-US014-1 verify + T-US014-2 + T-US015-1 done — 过期标记完整闭环**(3 task 1 提交)。**T-US014-1 verify**:由 T-US002-1 隐式实现完成,`taskListFilters.ts:192-193` `computeTaskBadge` 在 `task_date<today && !completed_at && !cancelled` 时返 `{kind:'overdue', label:'⚠ 已过期'}`;`TaskCard.tsx:189-197,283-284` 据此派生 `isOverdue` + 整卡 `COLOR_WARNING_BG`(#FBEAE6)底 + 左侧 4px `COLOR_WARNING`(#C95444)红竖条 + 红 badge chip;与启动 banner 设置无关永久显示 → status 标 ✅ Done(2026-09-23)。**T-US014-2**:详情页顶部过期 banner:`src/lib/createTaskForm.ts` 加 `formatOverdueHours(task, now)` 纯函数返 `{hours, displayText, showBanner}` — hours < 1 → "刚刚过期" / hours < 24 → "已过期 N 小时" / ≥ 24 → "已过期 N 天";showBanner 排除 cancelled / completed_at 非空 / task_date ≥ today 三种态。新组件 `src/components/OverdueBanner.tsx`(~50 行):圆角 12px,warning 浅底(#FBEAE6)+ warning 文字(#C95444),左 ⚠ icon + 中文案 + 右"补卡"按钮(Alert placeholder 留 T-US006),`accessibilityRole="alert"` + label "已过期 X,点击补卡",testID `overdue-banner`。`src/screens/TaskDetailScreen.tsx` ScrollView TaskHero 之上条件渲染(`isOverdue` 由 `computeTaskBadge` 派生 — 与 T-US014-1 红 chip / 红竖条同源,一致派生保证 banner 与卡片视觉同步)。**T-US015-1**:`src/services/ExpiryService.ts`(NEW,~120 行)`ExpiryWindow = 'yesterday_today' | 'this_week' | 'all' | 'off'` + `makeExpiryWindowPredicate(window, today)` 4 窗口谓词 + `filterExpiredTasks(tasks, window, today)`(先 `!cancelled && !completed_at && task_date < today` 排除,再过窗口) + `getExpiredTasksSummary(familyId, tasks, today)`(从 family_settings 单行 SELECT 读 `expiry_window` 字段,缺省 fallback `yesterday_today`,`maybeSingle<FamilySettingsRow>()`);`src/hooks/useExpiredTaskCount.ts`(NEW,~50 行)订阅 `useTasks()` + `useFamilyValue()`,deps `[family?.family.id, tasks, today]` 触发重算,响应 Realtime tasks 推送(SyncManager 已订阅 family_settings 也覆盖 settings 变更,本 hook 不显式监听 settings event);返回 `{count, loading}`,family null 时 0 + loading=false。`HomeScreen.tsx` 调用 hook(`console.log('[HomeScreen] expired tasks count:', count)` 占位,真正 banner 渲染留 T-US015-2)。**测试 4 个 new file**:`__tests__/OverdueBanner.test.tsx` 5 用例(`OverdueBanner` 组件契约 3 + props 完整性 1 + a11y label 1);`__tests__/ExpiryService.test.tsx` 8 用例(`makeExpiryWindowPredicate` 4 窗口 × 边界 / `filterExpiredTasks` 4 窗口 + 3 排除条件 / `getExpiredTasksSummary` 3 fallback);`__tests__/useExpiredTaskCount.test.tsx` 5 用例(family null / family off / yesterday_today 计数 / view 触发重算 / loading state);`__tests__/createTaskForm.test.tsx` 加 `formatOverdueHours` 6 用例(cancelled / completed / task_date≥today / hours<1 刚过期 / hours<24 整数 / days)。**验证**:`tsc --noEmit` 0 error;`jest --silent` 24+ new + 517 prior = 541+/541+ 全 pass(零回归)。**严格 scope**(dev self-acknowledge):❌ ExpiredTasksBanner 组件渲染 / N=0 不显示 / 点击跳过期列表(留 T-US015-2);❌ Banner 关闭状态持久化(留 T-US015-3);❌ 真正补卡 RPC(留 T-US006);❌ ExpiredTasksScreen 列表(留 T-US015-3);❌ family_settings Update Realtime 显式监听(SyncManager 已订阅);❌ T-US017-3 过期窗口设置 UI(Wave 5);❌ 改 SyncManager / LocalStore / AuthContext / FamilyContext / TaskService / CheckInService / FamilyService / useTasks / theme / package.json / jest config。**手动真机验证(必跑)**:创建 task with task_date = yesterday → 进 detail 看到 "已过期 X 小时" banner + 红 chip + 4px 红竖条;App 启动 → console.log 输出过期任务数(0 if 无过期)。变动文件:`document/tech-decision/task-breakdown-v1.0.md`(T-US014-1 status ✅)+ `src/lib/createTaskForm.ts`(M, +formatOverdueHours)+ `src/components/OverdueBanner.tsx`(NEW)+ `src/screens/TaskDetailScreen.tsx`(M, 集成 OverdueBanner)+ `src/services/ExpiryService.ts`(NEW)+ `src/hooks/useExpiredTaskCount.ts`(NEW)+ `src/screens/HomeScreen.tsx`(M, +useExpiredTaskCount console.log)+ `__tests__/createTaskForm.test.tsx`(M, +formatOverdueHours 6 cases)+ `__tests__/OverdueBanner.test.tsx`(NEW, 5 cases)+ `__tests__/ExpiryService.test.tsx`(NEW, 8 cases)+ `__tests__/useExpiredTaskCount.test.tsx`(NEW, 5 cases);US-014 进度 0/2 → 2/2(✅ Done)+ US-015 进度 0/3 → 1/3(✅ Done,启动过期任务查询服务层闭环,banner 渲染留 T-US015-2) |
| v1.0+us003-1-rev1 | 2026-09-23 | frontend-dev | **T-US003-1 review fix pass (1 Blocker + 3 Major)** — code-reviewer 评审 `5b715c3` 的 NEEDS FIXES verdict(1 Blocker 必修 + 4 Major + 10 Minor)修复,聚焦治本 Blocker + 一组相关 Major。**🔴 Blocker #1** 整个 create / edit task feature 不可用:`src/components/TaskFormBody.tsx` 整个 render 函数无任何 Button onPress 触发 `handleSave`,`grep -rn "onPress={handleSave}"` 0 匹配;`submitLabel` / `saveLabel` / `submitDisabledHint` 计算了但不渲染;后果:CreateTaskScreen header "保存" 是空白 `<Text accessibilityRole="button">{' '}</Text>`,EditTaskScreen header 是 `<Text color="transparent">{' '}</Text>`,**用户填完表单无任何方式提交 — TaskService.createTask / updateTask 永远不会被调**。**修复**(reviewer 推荐 A):TaskFormBody ScrollView 外加 `<XStack paddingHorizontal="$md" paddingVertical="$sm" borderTopColor>` 包 `<Button theme="active" flex={1} size="$md" borderRadius={12} onPress={handleSave} disabled={saveDisabled} accessibilityRole="button" accessibilityLabel={submitDisabledHint ?? saveLabel} accessibilityState={{disabled: saveDisabled}} testID="save-button">{saveLabel}</Button>` — sticky bottom 视觉 + a11y 完整(不用 `<View onTouchEnd>` 替代,reviewer Minor #9 已 flag 该模式 a11y 风险);CreateTaskScreen + EditTaskScreen header 删除占位 Text 改 `<View style={styles.headerRightSpacer} />`(保留 3 列对齐,header 不放保存按钮);沿用 Tamagui `theme="active"` 复用 pair-create 的赤陶主题色,与现有 Button 模式一致。**🟡 Major #2** `mode: TaskFormMode` prop 是死代码:`TaskFormBody.tsx:176,220,234` 的 `_mode` 只在 `console.error(\`mode=${_mode}\`)` 用于日志,create / edit 行为完全相同;**方案 A(已选 — 删 dead code)**:删 `mode` prop + `TaskFormMode` type + `_mode` 引用,CreateTaskScreen / EditTaskScreen 调用处去掉 `mode="create"` / `mode="edit"`,`handleSave` 日志改为 `[TaskFormBody] onSubmit threw`(无 mode 段);生产代码净减 -6 行(`TaskFormMode` type + `_mode` param rename + 3 处引用)。**🟡 Major #3-#5** `currentUserId` / `isOwner` 3 个 screen 各自用 `useFamilyValue()?.family.created_by ?? ''` 推断,与 service 层 `supabase.auth.getUser()` 走两套来源(2 人家庭 + 当前用户=创建者时巧合对,reviewer 标长期债务):**短期方案 — 加 helper 集中一处**:`src/contexts/FamilyContext.tsx` 加 `useCurrentUserId()` hook + 同步导出 `resolveCurrentUserId(family)` 1 行纯函数(`family?.family.created_by ?? ''`),为 jest 可测抽出纯函数;3 个 screen 调用点切换:`EditTaskScreen.tsx:121,148` + `CreateTaskScreen.tsx:246,250` + `TaskDetailScreen.tsx:159` 改用 `useCurrentUserId()`;**严格不**真正切到 `supabase.auth.getUser()`(留后续 polish,本任务只是抽 helper,沿用 family.created_by 与之前一致);JSDoc 明确 TODO 与长期契约(loading / no_family → `''`,in_family → `family.created_by`)。**测试**:`__tests__/FamilyContext.test.tsx` 新增 6 用例全 pass(`resolveCurrentUserId` 4:null → `''` / in_family → created_by / member 仍 created_by / stateless migration + `useCurrentUserId` 1:hook signature sanity + `useFamily` 1:Provider 外 throw 契约);同 InviteScreen.test.tsx + useTasks.test.tsx 风格不渲染组件(jest-expo + Tamagui ESM 限制);mock 整个 `../src/lib/supabase` 挡 env check(require 时炸,FamilyContext → AuthContext → supabase 链路)。**验证**:`tsc --noEmit` 0 error;`jest --silent` 343/343 pass(原 337 + 6 新,16 个 suite 全绿,零回归);FamilyContext 6/6,SyncManager 10/10,AuthService 17/17,TaskService 测试套全 pass。**手动真机验证**(Blocker #1 必须项):此 Windows env 无 `eas` CLI 且 `npx expo export --platform android` 触发已知 tamagui/jsx-runtime resolution carry-over(`T-SETUP-5` 末 / `T-SETUP-9` 末记)失败,**未跑真机 create / edit 流程**;TS 类型 + jest + 模式一致性(`theme="active"` 与 pair-create 同)+ `__tests__/FamilyContext.test.tsx` resolveCurrentUserId 4 case 全绿 共同作为 Blocker #1 修复的可验证证据。**用户后续在有 `eas` / 真实盘/EAS build 的环境必须手动跑**:`cd app/fam-schedule; npx expo start`(Expo Go)或 `eas build --profile development` 真机;create 流程:home → + → 填表 → tap 「保存」(sticky bottom) → 跳 home → 新 task 出现;edit 流程:点 task → 进 detail → ⋮ → 编辑 → 改 → tap 「保存」→ 跳回 detail → 数据更新。**严格 scope 不修**:🔴 Minor #6-#10 全部 defer T-FIX-06 hygiene batch(useEffect sync initialState dead path / MoreMenu dismiss layer / Alert + back race / onTouchEnd 替代 Button(本任务 Blocker #1 修复后已用 Tamagui Button 替代)/ header-save-placeholder accessibilityElementsHidden);真正改用 supabase.auth.getUser() 做 currentUserId 来源(留后续,本任务只抽 helper 沿用 family.created_by);SyncManager / LocalStore / AuthContext / FamilyService / theme / package.json / jest config 全不动。变动文件:`src/components/TaskFormBody.tsx`(M, +save button / -mode prop)+ `src/contexts/FamilyContext.tsx`(M, +useCurrentUserId / +resolveCurrentUserId)+ `src/screens/CreateTaskScreen.tsx`(M, -header save placeholder + useCurrentUserId)+ `src/screens/EditTaskScreen.tsx`(M, -header save placeholder × 2 + useCurrentUserId)+ `src/screens/TaskDetailScreen.tsx`(M, useCurrentUserId + isOwner 改用)+ `__tests__/FamilyContext.test.tsx`(NEW)+ 本文件;US-003 进度 1/2 → 2/2(✅ Done + review fix);**review round 2 验收重点**:save button render / useCurrentUserId 一致性 / 手动真机验证证据。 |
...
| 状态 | 数量 |
|---|---|
| ✅ Done(已完成) | 13(T-SETUP-1 + T-SETUP-1.1 + T-SETUP-2 + T-SETUP-4 + T-SETUP-5 + T-SETUP-6 + T-SETUP-7 + T-SETUP-9 + **T-FIX-01** + **T-FIX-02** + **T-FIX-03** + **T-US013-1** + **T-US012-2**)— **Sprint 1 SETUP 全清 + Wave 1 P0 修复 3/3 收尾 + US-013 启动 + US-012 邀请码生成** |
| 🟡 In Progress(进行中) | 1(T-SETUP-8 — EAS Build 排队中) |
| ⚪ Not Started(未开始) | 53(50 US/QA/Deploy + **3 T-FIX Review 修复**:T-FIX-04..06 P1)|
| 🔴 Blocked(阻塞中) | 0 |
| **合计** | **67** |
...
| US-013 匿名设备身份 | 2 | 2/2(**T-US013-1 ✅** AuthService + SecureStore 持久化层 + **T-US013-2 ✅** 启动守卫 retry-once + SplashScreen 抽出) | ✅ |
| US-012 创建/加入家庭 | 3 | **3/3 ✅**(T-US012-1 ✅ FamilyService + FamilyContext + Gate 收敛 + T-US012-2 ✅ 生成邀请码 UI + 倒计时 hook + createInvite RPC + **T-US012-3 ✅** 输入邀请码 6 格 UI + acceptInvite RPC + 加入后 Realtime 订阅)— **Sprint 1 E2E 闭环达成** | ✅ |
| US-001 创建任务 | 2 | 0/2 | ⚪ |
| US-002 查看任务列表 | 3 | 0/3 | ⚪ |
...
- **Priority**: P0
- **Assignee**: frontend-dev
- **Status**: ✅ Done(2026-09-17)— 启动守卫 retry-once + SplashScreen 抽出 + 错误屏重试 UI,详见 ChangeLog `v1.0+us013-2`
- **Definition of Done**:
...
- **Priority**: P0
- **Assignee**: frontend-dev
- **Status**: ✅ Done(2026-09-22)— 详见 ChangeLog `v1.0+us012-3`
- **Definition of Done**:
  - 6 位数字输入框(自动跳到下一位)
  - 调 accept_invite RPC
...
- **Priority**: P1
- **Assignee**: frontend-dev
- **Status**: ✅ Done(2026-09-23)— 由 T-US002-1 隐式实现:`computeTaskBadge` 在 `task_date < today && !completed_at && !cancelled` 时返回 `{kind:'overdue', label:'⚠ 已过期'}`(`taskListFilters.ts:192-193`);`TaskCard.tsx` 据此派生 `isOverdue` + 整卡 `COLOR_WARNING_BG`(#FBEAE6)底 + 左侧 4px `COLOR_WARNING`(#C95444)红竖条 + 红 badge chip;与启动 banner 设置无关永久显示。详见 ChangeLog `v1.0+us014-1`
- **Definition of Done**:
